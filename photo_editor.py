...
class ModificationTab(QWidget):
    ...
    def on_width_change(self, e):
        ...
        if self.ratio_check.isChecked():
            r_height = _get_ratio_height(
                _img_original.width, _img_original.height, int(self.width_box.text()))
    ...
    def on_height_change(self, e):
        ...
        if self.ratio_check.isChecked():
            r_width = _get_ratio_width(
                _img_original.width, _img_original.height, int(self.height_box.text()))
    ...
    def on_apply(self, e):
        ...
        operations.size = int(self.width_box.text()), int(
...
class AdjustingTab(QWidget):
    ...
    def on_brightness_slider_released(self):
        logger.debug(
            f"brightness selected value: {self.brightness_slider.value()}")
    ...
    def on_upload(self):
        ...
        if img_path:
            ...
            if _img_original.width < _img_original.height:
                w = THUMB_SIZE
                h = _get_ratio_height(
                    _img_original.width, _img_original.height, w)
            else:
                h = THUMB_SIZE
                w = _get_ratio_width(_img_original.width,
                                     _img_original.height, h)
